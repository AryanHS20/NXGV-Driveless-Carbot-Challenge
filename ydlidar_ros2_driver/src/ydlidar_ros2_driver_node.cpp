...
int main(int argc, char *argv[]) {
  ...
  // FIX: Added default values to all declare_parameter calls
  ...
  if (ret)
  {
    ...
    int i_v = 0;
    node->declare_parameter("m1_mode", i_v); // FIX: Added default 0
    node->get_parameter("m1_mode", i_v);
    laser.setWorkMode(i_v, 0x01);
    i_v = 0;
    node->declare_parameter("m2_mode", i_v); // FIX: Added default 0
    node->get_parameter("m2_mode", i_v);
    laser.setWorkMode(i_v, 0x02);
    i_v = 1;
    node->declare_parameter("m3_mode", i_v); // FIX: Added default 1
