...
class ServoControllerV9(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('recorded_parking_duty_limit', 100.0)
        ...
        self.declare_parameter('publish_encoder_diagnostics', False)
        ...
        self.encoder_diag_pub = self.create_publisher(
            String, '/r5/encoder_diagnostics', 10)
        ...
        self.parking_start_wait = False
        self.create_subscription(Bool, '/parallel_parking_wait', self._parking_wait_cb, 10)
        ...
        self._last_encoder_diag_t = 0.0
        self._encoder_rejected_counts = [0, 0, 0, 0]
        self.publish_encoder_diagnostics = bool(
            self.get_parameter('publish_encoder_diagnostics').value)
    ...
    def _on_params(self, params) -> SetParametersResult:
        ...
        for p in params:
            ...
            if p.name in ('auto_motor_duty_limit', 'recorded_parking_duty_limit') and not 0 < p.value <= 100:
    ...
    def joy_callback(self, msg: Joy) -> None:
        ...
        if rose(11) or rose(4): 
            if self.rp_state == 'PLAYBACK' and self.playback_kind in ('parallel', 'perpendicular'):
                # One press cancels parking and leaves the car in MANUAL.
                self._stop_playback('manual_takeover')
                self.prev_buttons = list(msg.buttons)
                self.prev_axes = list(msg.axes)
                return
            ...
            if self.manual_mode:
                ...
                # Wait for a new permit after the operator selects AUTO.
                self.motion_permitted = False
                self.last_permit_time = 0.0
                self.auto_cmd_stale_reported = False
            self._update_dash()
    ...
    def cmd_vel_auto_callback(self, msg: Twist) -> None:
        ...
        if not self.manual_mode or self.rp_state == 'PLAYBACK':
            # Keep the recorded start pose while the sign confirms in AUTO.
            # Playback itself bypasses this hold and remains safety filtered.
            self.process_twist(Twist() if self.parking_start_wait and self.rp_state != 'PLAYBACK' else msg)

    def _parking_wait_cb(self, msg):
        self.parking_start_wait = bool(msg.data)
        if self.parking_start_wait and not self.manual_mode and self.rp_state != 'PLAYBACK':
            self.stop_robot()
    ...
    def _permit_callback(self, msg):
        ...
        if not self.motion_permitted and (not self.manual_mode or self.rp_state == 'PLAYBACK'):
            self._stop_for_interlock('motion permission denied')
        elif self.motion_permitted:
            self._last_auto_hold_reason = ''

    def _motion_allowed(self):
        if self.estop:
            return False
        if (self.rp_state == 'PLAYBACK'
                and self.playback_kind in ('parallel', 'perpendicular')
                and self.manual_mode):
            return False
        if not self.joy_unlocked or not 0 <= time.monotonic() - self.last_joy_time <= self.joy_timeout:
            return False
    ...
    def _stop_for_interlock(self, reason):
        """Hold zero in selected AUTO for permission waits; abort real faults.

        MANUAL has a false safety permit. Forcing MANUAL on that same false
        permit during an AUTO transition prevented the handshake completing.
        Keeping mode selected never authorizes motor output.
        """
        now = time.monotonic()
        controller_fresh = (self.joy_unlocked
                            and 0 <= now - self.last_joy_time <= self.joy_timeout)
        command_fresh = (self.last_auto_cmd_time > 0
                         and 0 <= now - self.last_auto_cmd_time
                         <= float(self._param_cache['auto_cmd_timeout']))
        permit_fresh = (self.motion_permitted and self.last_permit_time > 0
                        and 0 <= now - self.last_permit_time <= 0.4)
        if (not self.manual_mode and self.rp_state != 'PLAYBACK'
                and not self.estop and controller_fresh and command_fresh
                and not permit_fresh):
            self.stop_robot()
            if getattr(self, '_last_auto_hold_reason', '') != reason:
                self.get_logger().warn('AUTO selected, motors held at zero: ' + reason)
                self._last_auto_hold_reason = reason
            return
    ...
    def _abort_motion(self, reason):
        ...
        self.stop_robot()
        if not self.manual_mode:
            self.get_logger().warn('Forced MANUAL stop: ' + reason)
            self.manual_mode = True
            self.auto_mode_pub.publish(Bool(data=False))
    ...
    def process_twist(self, msg: Twist) -> None:
        ...
        duty_param = ('recorded_parking_duty_limit'
                      if self.rp_state == 'PLAYBACK' and self.playback_kind in ('parallel', 'perpendicular')
                      else 'auto_motor_duty_limit')
        duty_limit = max(0, min(100, int(self.get_parameter(duty_param).value)))
    ...
    def _encoder_read_loop(self) -> None:
        ...
        try:
            ...
            valid_d_ticks = []
            for channel, d in enumerate(d_ticks):
                if abs(d) > jump_thresh:
                    valid_d_ticks.append(0.0)
                    self._encoder_glitch_count += 1
                    self._encoder_rejected_counts[channel] += 1
                else:
            ...
            if (self.publish_encoder_diagnostics and
                    now - self._last_encoder_diag_t >= 0.2):
                self.encoder_diag_pub.publish(String(data=json.dumps({
                    'stamp_s': self.get_clock().now().nanoseconds * 1e-9,
                    'ticks': [int(value) for value in ticks],
                    'delta_ticks': [int(value) for value in d_ticks],
                    'rejected_total': self._encoder_rejected_counts,
                    'selected_channel': motor_idx,
                    'motor_pwm': int(self.target_motor_val),
                    'dt_s': round(dt, 4),
                }, separators=(',', ':'))))
                self._last_encoder_diag_t = now
    ...
    def _stop_playback(self, result='aborted') -> None:
        ...
        named_parking = self.playback_kind in ('parallel', 'perpendicular')
        ...
        self.stop_robot()
        if named_parking and not self.manual_mode:
            # A completed/aborted parking trace must not fall through to V4
            # lane commands while the vehicle is inside a parking bay.
            self.manual_mode = True
            self.auto_mode_pub.publish(Bool(data=False))
    ...
    def stop_robot(self) -> None:
        ...
        try:
            self.bot.set_motor(0, 0, 0, 0)
            self.bot.set_pwm_servo(self.servo_steer_id, self.servo_center)
            # Keep the write cache consistent with the actual stop. Otherwise
            # resuming the same pre-hold command can skip its hardware write.
            self.sent_motor_val = 0
            self.sent_servo_val = self.servo_center
            self.last_hw_send_time = time.monotonic()
        except Exception:
