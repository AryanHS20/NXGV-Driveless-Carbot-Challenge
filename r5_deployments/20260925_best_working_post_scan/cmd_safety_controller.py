...
class CmdSafetyController(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('auto_trial_limit_sec', 0.0)
        self._trial_limit = float(self.get_parameter('auto_trial_limit_sec').value)
        if not math.isfinite(self._trial_limit) or not 0 <= self._trial_limit <= 2.0:
            raise ValueError('auto trial limit must be 0 (disabled) or at most 2 seconds')
        self._trial_mode = None
        self._trial_mode_stamp = 0.0
        self._trial_saw_manual = False
        self._trial_deadline = None
        self._trial_expired = False
        self.create_subscription(Bool, '/auto_mode', self._trial_mode_cb, 10)
    ...
    def _on_params(self, params) -> SetParametersResult:
        ...
        for p in params:
            if p.name in ('track_test_mode', 'auto_trial_limit_sec'):
    ...
    def _trial_mode_cb(self, msg):
        now = time.monotonic()
        active = bool(msg.data)
        if not active:
            self._trial_saw_manual = True
            self._trial_deadline = None
            self._trial_expired = False
        elif self._trial_mode is not True:
            self._trial_deadline = now + self._trial_limit if self._trial_saw_manual else None
            self._trial_expired = not self._trial_saw_manual
            self._trial_saw_manual = False
        self._trial_mode = active
        self._trial_mode_stamp = now

    def _trial_hold_reason(self, now):
        if self._trial_limit <= 0:
            return ''
        if self._trial_mode is not True:
            return 'trial waiting for AUTO after MANUAL'
        if now - self._trial_mode_stamp > 0.5:
            self._trial_expired = True
        if self._trial_deadline is None or now >= self._trial_deadline:
            self._trial_expired = True
        if self._trial_expired:
            return 'trial ended; return to MANUAL'
        return ''
    ...
    def _control_loop(self) -> None:
        ...
        trial_reason = self._trial_hold_reason(now)
        if trial_reason:
            self._last_reason = trial_reason
        permit = sensors_ok and not self.estop and not stale and clearance_ok and not trial_reason
    ...
    def _publish_status(self) -> None:
        ...
        payload = {
            'auto_trial_limit_sec': self._trial_limit,
            'trial_expired': self._trial_expired,
            'estop': self.estop,
