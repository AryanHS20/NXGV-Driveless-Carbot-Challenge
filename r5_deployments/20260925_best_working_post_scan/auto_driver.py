...
class AutoDriver(Node):
    ...
    def __init__(self):
        ...
        # Fused heading and lane curvature for path planning
        self.fused_heading_rad = 0.0
        self.lane_curvature = 0.0
        self.create_subscription(Float32, '/fused_heading', self._fused_heading_callback, 10)
        self.create_subscription(Float32, '/lane_curvature', self._lane_curvature_callback, 10)
    ...
    def _lane_curvature_callback(self, msg: Float32) -> None:
    ...
    def _lane_follow_cmd(self) -> Twist:
        ...
        # --- Adaptive speed: slower in turns, faster on straights + curvature regulation ---
        scale = self._param_cache['speed_error_scale']
        min_spd = self._param_cache['min_turn_speed']
        curv_factor = 1.0 / (1.0 + 2.5 * abs(self.lane_curvature))
        speed_mult = max(min_spd, (1.0 - scale * abs(error)) * curv_factor)
    ...
    def publish_cmd_vel(self) -> None:
        ...
        if not self.in_auto_mode:
            ...
            # Track test: keep the mission in lane follow while a confirmed
            # camera gate blocks the lane, so lane loss cannot enter recovery.
            ...
            elif self.boom_gate_last_time > 0.0 and not self.boom_gate_open:
                target, reason = ChallengeState.LANE_FOLLOW, 'WAITING FOR BOOM GATE'
