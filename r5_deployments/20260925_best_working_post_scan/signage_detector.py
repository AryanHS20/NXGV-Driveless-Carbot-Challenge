...
from .boom_gate_camera_core import BoomGateDebounce, red_arm_crosses_lane
...
class SignageDetector(Node):
    ...
    def __init__(self):
        ...
        # Keep camera gate publication off until the stationary venue check.
        # V4 holds zero if the gate topic has no fresh confirmed observation.
        self.declare_parameter('publish_boom_state', False)
        # Read-only deployment keeps the existing mission advisory topics quiet.
        self.declare_parameter('publish_advisories', True)
        ...
        self._cnt_yellow = 0
        self._cnt_lamp = 0
        self.boom_gate_open = None  # Three camera frames confirm the first state.
        self._boom_camera = BoomGateDebounce(confirmation_frames=3)
        ...
        self.hill_frame_pub = self.create_publisher(Bool, '/hill_model_visible', 10)
        ...
        # Publish only confirmed physical camera-arm state when enabled.
        ...
        self.detections_pub = self.create_publisher(
            String, '/v4_experimental/signage/detections', 10)
    ...
    def _update_param_cache(self) -> None:
        self._param_cache = {
            ...
            'publish_advisories': bool(self.get_parameter('publish_advisories').value),
        }
    ...
    def publish_states(self) -> None:
        ...
        if not valid:
            ...
            self.traffic_light_active = 'unknown'
            self.parking_kind = ''
        if not self._param_cache['publish_advisories']:
            return
        ...
        if (self._param_cache['publish_boom_state'] and valid
                and self.boom_gate_open is not None):
    ...
    # ── Main callback ────────────────────────────────────────────────
    def publish_detections(self, image_msg: Image, boxes, scores, class_ids) -> None:
        """Publish frame-stamped boxes; no range or vehicle bearing is assumed."""
        ...
        for box, score, class_id in zip(boxes, scores, class_ids):
            ...
            if not 0 <= cid < len(CLASS_NAMES):
                continue
            coords = [max(0.0, min(1.0, float(value) / 640.0)) for value in box]
            detections.append({
                'class_id': cid, 'class_name': CLASS_NAMES[cid],
                'confidence': round(float(score), 4),
                'box_norm_xyxy': [round(value, 5) for value in coords],
                'center_norm_xy': [round((coords[0] + coords[2]) / 2.0, 5),
                                   round((coords[1] + coords[3]) / 2.0, 5)],
            })
        payload = {
            'stamp_s': float(stamp.sec) + float(stamp.nanosec) * 1e-9,
            'image_width': int(image_msg.width),
            'image_height': int(image_msg.height),
            'coordinate_space': 'normalized raw image after model resize',
            'camera_to_vehicle_validated': False,
    ...
    def image_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            if self._param_cache['publish_boom_state']:
                self.boom_gate_open = self._boom_camera.observe(
                    red_arm_crosses_lane(bgr))
            ...
            self.publish_detections(msg, final_boxes, final_scores, final_cids)
    ...
    def _update_states(self, boxes, class_ids, verdicts=None) -> None:
        ...
        self.hill_frame_pub.publish(Bool(data=1 in cids))
        ...
        self._cnt_speed_bump, self.speed_bump_active = self._bump(
            6 in cids, self._cnt_speed_bump, self.speed_bump_active)
        ...
        # The archived YOLO boom classes are unvalidated. Only the red-arm
        # camera observation may set the gate state when enabled at the venue.
