...
from .observed_centerline_core import select_observed_centerline
from .adaptive_centerline_core import AdaptiveCenterline
from .lane_reference import live_steering_reference
from .bev_core import CalibrationError, load_profiles, profile_report, warp_to_bev
from .near_field_support import visible_corridor_bootstrap
...
class TrajectoryShadow(Node):
    def __init__(self) -> None:
        ...
        if self._lane_controller not in ('filtered_centerline', 'live_lane_arc',
                                        'observed_centerline', 'adaptive_centerline'):
            raise ValueError('unknown lane controller')
        self._auto_mode = False
        self._adaptive_centerline = AdaptiveCenterline()
        self.declare_parameter('minimum_servo_step_units', 0)
        self.declare_parameter('servo_range_left_units', 50)
        self.declare_parameter('servo_range_right_units', 70)
        self._minimum_servo_step = int(self.get_parameter('minimum_servo_step_units').value)
        self._servo_range_left = int(self.get_parameter('servo_range_left_units').value)
        self._servo_range_right = int(self.get_parameter('servo_range_right_units').value)
        if not (0 <= self._minimum_servo_step <= min(self._servo_range_left, self._servo_range_right)
                and min(self._servo_range_left, self._servo_range_right) > 0):
            raise ValueError('invalid servo response mapping')
    ...
    def _on_parameters(self, parameters) -> SetParametersResult:
        ...
        protected = {
            'minimum_servo_step_units', 'servo_range_left_units', 'servo_range_right_units',
            'lane_controller', 'enabled', 'track_test_mode', 'enforce_road_support_in_track_test',
    ...
    def _mode_callback(self, msg: Bool) -> None:
        ...
        self._selected = None
        self._adaptive_centerline.reset()
    ...
    def _try_process(self) -> None:
        ...
        try:
            ...
            if self._lane_controller == 'adaptive_centerline':
                scan_fresh = (self._scan_mono is not None and
                              now-self._scan_mono <= self._scan_timeout)
                imu_fresh = self._imu_yaw_rad is not None and now-self._imu_mono <= .35
                self._selected, self._last_candidates = self._adaptive_centerline.select(
                    raw_corridor, self._mask, self._profiles['primary'],
                    self._geometry, self._config, now=now,
                    imu_yaw=self._imu_yaw_rad if imu_fresh else None,
                    last_steer=self._last_control_steer_rad,
                    minimum_servo_step=self._minimum_servo_step,
                    servo_range_left=self._servo_range_left,
                    servo_range_right=self._servo_range_right,
                    obstacles=self._scan_points if scan_fresh else ())
                self._last_error = ('' if self._selected else
                                    self._last_candidates[0]['reject_reason'])
                self._last_warning = ''
                self._last_processed_stamp = expected_stamp
                self._processed_frames += 1
                if self._selected:
                    self._last_control_steer_rad = self._selected[
                        'command_steer_rad_diagnostic_only']
                    self._last_success_mono = now
                return
            if self._lane_controller == 'observed_centerline':
                self._selected, self._last_candidates = select_observed_centerline(
                    raw_corridor, self._geometry, self._config,
                    minimum_servo_step=self._minimum_servo_step,
                    servo_range_left=self._servo_range_left,
                    servo_range_right=self._servo_range_right)
                self._last_error = ('' if self._selected else
            ...
            if self._lane_controller == 'live_lane_arc':
                profile = self._profiles['primary']
                if getattr(self, '_coverage_profile', None) is not profile:
                    _, self._bootstrap_coverage = warp_to_bev(
                        np.zeros((profile.resolution[1], profile.resolution[0], 3), np.uint8), profile)
                    self._coverage_profile = profile
                near_field = visible_corridor_bootstrap(
                    raw_corridor, self._geometry, self._config.near_field_max_gap_m,
                    self._config.near_field_settle_m, profile, self._bootstrap_coverage)
            ...
            if self._lane_controller == 'live_lane_arc':
                reference = live_steering_reference(
                    steering_rows, self._config.expected_lane_width_m,
                    self._config.horizon_m, self._config.step_m)
                self._process_live_arc(reference, near_field, now, expected_stamp)
                return
        ...
        if selected is not None:
            ...
            # Validate the exact steering after filtering and direction hold.
            # Candidate validity cannot certify this independently chosen command.
            sent = evaluate_steering_command(
            ...
            selected['sent_command_road_blocked_steps'] = sent.road_blocked
            selected['sent_command_obstacle_blocked_steps'] = sent.obstacle_blocked
            selected['sent_command_minimum_support'] = round(sent.minimum_support, 4)
            selected['sent_command_endpoint_m'] = {
                'forward': round(sent.points[-1].x, 4),
                'left': round(sent.points[-1].y, 4),
            }
            if sent.obstacle_blocked or (
                    sent.road_blocked and (not self._track_test_mode
                                          or self._enforce_track_test_road_support)):
                self._selected = None
                self._last_error = ('sent steering intersects an obstacle' if sent.obstacle_blocked
                                    else 'sent steering leaves observed road')
                self._last_warning = ''
                self._last_processed_stamp = expected_stamp
                self._processed_frames += 1
                return
    ...
    def _publish_status(self) -> None:
        ...
        payload = {
            ...
            'use_lidar_obstacles': self._use_lidar_obstacles,
