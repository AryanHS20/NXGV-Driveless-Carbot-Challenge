...
present above --conf. Class count is read from the model outputs; labels
follow tools/bpu_model/model_manifest_unified14.json.
"""
import argparse
import time
...
NAMES_14 = ['end_of_tunnel_sign', 'hill_sign', 'obstacle_sign',
            'parallel_parking_sign', 'perpendicular_park_sign',
            'roundabout_sign', 'speed_bump_sign', 'traffic_warn_sign',
            'tunnel_sign', 'traffic_red', 'traffic_yellow', 'traffic_green',
            'boom_closed', 'boom_open']
...
def main():
    ...
    for li, s in enumerate(STRIDES):
        ...
        cls = np.squeeze(cls)
        if cls.ndim == 3 and cls.shape[0] in (10, 14, 16):  # NCHW -> HWNC
            cls = np.transpose(cls, (1, 2, 0))
        if not (cls.ndim == 3 and cls.shape[-1] in (10, 14, 16)):
            print(f'stride {s}: unexpected cls shape {cls.shape}, skipping')
            continue
        ncls = cls.shape[-1]
        names = NAMES_14 if ncls == 14 else NAMES_16
